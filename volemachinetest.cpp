...
public:
    ...
    string floating_point_sum(string bin1, string bin2){
    double num1_dec = BintoDec(bin1);
    double num2_dec = BintoDec(bin2);
    double sum = num1_dec + num2_dec;

}
string twosComplements_sum(string bin1, string bin2) {
    // Convert binary strings to integers (treat them as signed 8-bit numbers)
    int8_t num1 = static_cast<int8_t>(bitset<8>(bin1).to_ulong());
    int8_t num2 = static_cast<int8_t>(bitset<8>(bin2).to_ulong());

    // Perform addition
    int8_t sum = num1 + num2;
